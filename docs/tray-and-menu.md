...
This document details the technical architecture of the **WinSpaces tray surface**: the runtime-generated Fluent badge icon (`tray.rs`) and the custom-drawn Windows 11 acrylic context menu (`menu.rs`) — including the Win32 mechanics that make the menu look native and the design constraints that keep both effectively free at runtime.
...
## 2. Context Menu Architecture (`menu.rs`)

On Windows 11 (build ≥ 22000) the context menu is **not** an `HMENU`. It is a hand-drawn `WS_POPUP` window styled like a native Windows 11 flyout — the same approach PowerToys uses for its overlay surfaces, and the only way to get acrylic, custom row metrics, and per-item Fluent glyphs without a UI framework. Older builds fall back to the classic `TrackPopupMenu` menu (`show_tray_menu_legacy` in `main.rs`, following the OS apps mode via uxtheme ordinal 135 `SetPreferredAppMode(AllowDark)`).
...
| Light/dark | `DWMWA_USE_IMMERSIVE_DARK_MODE` per resolved theme (also steers the acrylic tint). The palette (`menu_theme`) is resolved on **every open** from the shared preference — `settings_ui::theme`: App Theme selector at `HKCU\Software\WinSpaces\GuiTheme`, falling back to the OS apps mode — so the flyout tracks live theme changes with no hooks or resident state |
...
Everything is plain GDI — no Direct2D, no GDI+. The trick is manual alpha management in a 32bpp premultiplied DIB (`render_window`):

1. The DIB is filled *directly* (slice write, not GDI) with the premultiplied background tint — `#2C2C2C` dark / `#F9F9F9` light, at alpha 232 — so the acrylic blur shows through it.
...
A non-activating popup cannot use the normal focus-based input model, and `SetCapture` is only honored for the foreground thread — which a background daemon usually is not. Three mechanisms replace it, all scoped to the menu's lifetime:

| Signal | Mechanism | Notes |
| :--- | :--- | :--- |
| Keyboard | The daemon's **existing** `WH_KEYBOARD_LL` hook (`low_level_keyboard_proc`) | While the menu is open, nav keys (`Esc`/arrows/`Enter`/`Space`) are re-posted to the menu window and swallowed; `Win`/`Alt` close the menu (swallowing the keydown also stops the Start menu from opening on release). Adds one boolean check per keystroke when the menu is closed. |
...
| Focus loss | Owner `WM_ACTIVATE(WA_INACTIVE)` forwarding (`main.rs`) | `SetForegroundWindow(owner)` runs before showing the menu; Alt-Tab or clicking another app deactivates the owner, which light-dismisses the menu. |
...
Measured on a live daemon (cycle-accurate, two monitors, 2.5K display):

| State | Private memory | CPU |
| :--- | :--- | :--- |
| Idle (menu closed) | ~2.4 MB | ~0.8 Mcycles / 5 s ≈ 0.004% of a core |
| Menu open, per hover repaint | no retained delta | ~3.5 Mcycles ≈ 0.9 ms |
| Menu open → closed | GDI 18→22→18, USER 19→22→19 | all resources returned |
...
5. **Zero new dependencies.** `menu.rs` uses only `windows-sys` features that were already linked (plus `Win32_UI_Controls` for the `MARGINS` struct).
