# Task View, Overview & Win+Tab Interception

This document details the technical architecture, Win32 API mechanics, and implementation for the **WinSpaces Overview** overlay, system shortcut hijacking (`Win+Tab`), tray icon triggers, and window lifecycle management.
...
WinSpaces includes a custom, GPU-accelerated **macOS-style Overview** rendered natively by the background Rust daemon:
...
- **Live Hardware Video Thumbnails**: Uses Windows Desktop Window Manager (`DwmRegisterThumbnail` + `DwmUpdateThumbnailProperties`) to project live, zero-copy 60+ FPS window textures directly onto the overview grid.
...
- **Top Spaces Bar**: Displays one interactive card per space on the active monitor (each monitor has its own dynamic count, 1–9) with live window counts and active space badges (`#818CF8`). Switching spaces while the overlay is open — by clicking a Space card, pressing `1`–`9`, or hitting the global switch/move hotkeys (e.g. `Alt+3`) — keeps Overview open: `refresh_overview` re-syncs the spaces bar and thumbnail grid **in place** (no hide/show, so no flash). Switching to the already-active space is a no-op. Only `Esc`, a backdrop click, or focusing a window dismisses the overlay. When the natural strip would overflow a narrow monitor, `spaces_bar_metrics` shrinks card widths toward a floor instead of letting the strip run off-screen.
- **Reordering Spaces (macOS-style)**: Users can click and drag any Space card horizontally across the top bar to reorder spaces on that display, or press `Ctrl+Shift+←/→` to walk the active space one slot (a no-op at either end). Only horizontal travel past `SM_CXDRAG` lifts a card — reordering is a horizontal gesture, so a vertical nudge must not detach it. While dragging, sibling cards shift to illustrate the new order, a placeholder marks the vacated slot, and the dragged card floats with an accent border (`#818CF8`), pinned to the bar's `y` because DWM composites the thumbnail grid *above* this window's GDI output. It is painted after every other tile in the bar, including the "New Space" tile, so it rides over what it passes instead of sliding underneath. `Esc` cancels a live drag without dismissing the overlay. Only the pointer's x decides the drop — the card is pinned to the bar, so the cursor is free to roam anywhere over the overlay without losing the move. Dropping reorders monitor space membership in `SpaceManager::reorder_space`, preserves active space identity and window tracking, updates the Fluent tray badge, and refreshes the overlay in place without flashing. Drag frames invalidate the spaces-bar strip (`spaces_bar_strip_rect`) — always the whole strip, never the band between two card positions, because the throttle below drops frames, so the last *painted* position is not the previous message's position and a band measured from the latter leaves a trail of ghost cards behind. Paints are throttled to one blit per display frame (`frame_interval_ms` from `GetDeviceCaps(VREFRESH)`, with a one-shot `TIMER_DRAG_PAINT` for the trailing position). A mouse reports several times faster than the panel refreshes, and painting a DWM-composited window more than once per frame lets the compositor sample a half-written surface — which reads as the card tearing across a scanline.
- **Add / Remove Spaces (macOS-style)**: A "New Space" tile after the last space card appends a space (hidden at the 9-space cap). It is deliberately *not* in the space cards' visual family — a dimmer background behind a dashed border, with the same Segoe Fluent Icons "Add" glyph (`GLYPH_ADD`) over a caption, tinting indigo on hover. Label and glyph both match the tray menu's New Space item, so the action reads the same in both surfaces. In card colours with a bare "+" it read as an N+1'th space rather than an action. Hovering a space card reveals an × close button (hidden when it is the monitor's last space) that removes the space — its windows migrate to the space on the left (the first space's windows fall right) and indices shift down. The tile deliberately lives *outside* `space_cards` (a separate `plus_rect`), so every consumer of that vector can assume it contains real spaces only; the drop handler resolves targets by the card's `desk_idx`, never its vector position. Both actions funnel through the bin's `add_space_on` / `remove_space_on` choke points via the `OverviewHost.add_space` / `OverviewHost.remove_space` entries (§1.1), which persist the count, re-register digit hotkeys when the cross-monitor maximum changes, update the tray, and refresh the open overlay.
- **Drag-and-Drop Relocation**: Users can click and drag any window thumbnail card and drop it onto a Space card in the top bar to relocate it to that space — or onto the "New Space" tile to create a new space with that window on it.
- **Closing a Window**: Hovering a window card reveals an × button in its header (the title reserves the room only while it shows, so no title is ever clipped by a button that isn't there), tinting destructive red under the pointer. Unlike the space close button — which acts on the button *down*, because removing a space only migrates its windows — this one waits for the button-up over the same circle: closing an app is not undoable, and a misclick has to keep its way out by sliding off before releasing. A cancelled press is swallowed rather than falling through to the card beneath it, which would focus the window it just declined to close. The action posts `WM_CLOSE` through `OverviewHost.close_window` and changes no daemon state at all. **It deliberately does not untrack the window**: `WM_CLOSE` is a *request*, and an app with unsaved work answers it with a confirmation dialog it may never dismiss — untracking on the click would clear the state prop and discard the space assignment of a window that is still alive. Instead the close arms `TIMER_CLOSE_VERIFY` (150 ms), which rebuilds the grid once the app has had its message cycle: the eligibility filter drops the card if the window really died, and keeps it if the app is still sitting on a dialog. The card disappearing therefore *means* the window went away, rather than meaning the click was received.

That verify pass is the only prompt signal available, not a belt-and-braces extra. `HSHELL_WINDOWDESTROYED` (§5) is not a substitute: the shell fires it while the handle can still be alive, and the liveness guard that makes that path safe — our own cloaking makes the shell fire it too — then drops the event with no retry, leaving the card to sit there until some later scan happens to prune it. One consequence worth knowing: such a confirmation dialog opens *behind* the topmost overlay, so it surfaces when Overview is dismissed.

- **Pinning a Window to Every Space (sticky windows)**: The same hover header carries a pin button to the left of the ×, and `P` over a hovered card does the same thing. A pinned window shows on every space of *its display* — the pin is per-monitor, because a window only ever sits in one monitor's space list. The button follows the close button's arm-on-down / fire-on-up-over-the-same-circle contract rather than the space close button's act-on-down one: the toggle is trivially undoable, but the two circles sit six pixels apart and a slip between them must not close an app. The mark is `GLYPH_PIN` (outline) while unpinned and `GLYPH_PIN_FILLED` while pinned, so the state reads from the glyph and not only from its indigo fill; a resting (unhovered) pinned card keeps the filled glyph as a bare badge with no circle behind it, which shows the state without advertising a target that is not hit-tested until the card is hovered. Pinned windows are folded into every space's card grid and window count by `SpaceManager::windows_for_space`, which walks the monitor's spaces in order rather than iterating the `sticky_windows` `HashSet` — set iteration order shifts on rehash, which would jitter a pinned card's grid slot whenever some unrelated window was pinned.

Dragging a pinned card to another space card is a normal relocation: it changes which space the window is *tracked* on and keeps the pin, so nothing visibly moves. That falls out of `track_window` detaching rather than removing (see `manager.rs`), and it is the reason the pin is not silently lost every time the scan re-homes a window across monitors.
- **Dismissal does not re-activate anything.** `hide_overview` hides the overlay and stops; the shell promotes whatever sits beneath a topmost window that goes away. Explicitly restoring the pre-overlay foreground window is a trap worth naming, because it looks like an obvious polish: that window's space may have changed *while* the overlay was up (the user switched spaces from the spaces bar), and activating it fires the very `HSHELL_WINDOWACTIVATED` path in §4 that switches a monitor to the activated window's space — silently undoing the switch the user just made, on every `Esc` more than 500 ms after it (the `suppress_foreground_until` window).

---

### 1.1 The `OverviewHost` Indirection

Overview lives in `winspaces-ui` and never sees the bin's application state. Its entry points (`toggle_overview`, `show_overview`, `refresh_overview`) take `&mut SpaceManager` directly — enough for everything the overlay does on its own (scanning untracked windows, reading monitor/space data, rebuilding the card grid). What it *cannot* do itself — because the action must also update the persisted space count, re-register digit hotkeys, refresh the tray badge, or otherwise touch state a UI crate must not depend on — goes through `OverviewHost`, a plain struct of nine `fn` pointers (`add_space`, `remove_space`, `reorder_space`, `reorder_space_neighbor`, `switch_space`, `move_window_to_space`, `move_window_to_new_space`, `close_window`, `toggle_window_sticky`). The bin builds one static `OverviewHost` whose functions each wrap the exact `with_app_state` block that used to sit inline in this module, and installs it once at startup (`install_host`); Overview reaches it through a private `OnceLock`.

**Why fn pointers instead of posted messages.** The obvious alternative — have Overview `PostMessage` its intent to the daemon's message window — is architecturally cleaner but defers the action to the next message-loop turn. Today, `WM_LBUTTONUP` completes a space reorder *and* the resulting overlay refresh synchronously, before the handler returns. Deferring either half through `PostMessage` would change the frame in which the overlay repaints after a drop — visible as a stray flash or a stale frame — which is exactly the class of bug a test suite cannot catch. A direct `fn` call preserves the original synchronous behavior; only the *name* Overview calls through changed, not when the call happens or what state it borrows.

This is also why the previous shape — the bin and Overview calling into each other directly — was a genuine bidirectional dependency cycle, not just an inconvenience: the bin needed Overview's refresh function, and Overview needed the bin's state-mutating helpers. Splitting them into separate crates forced the inversion: the crate boundary means the bin's helpers now flow *into* Overview as data (function pointers) rather than Overview naming the bin's functions directly, so the dependency only ever points from `winspaces` down to `winspaces-ui`, never back.

**The geometry payoff.** Overview's hit-testing and layout math — spaces-bar metrics, card rects, drop targets — live in a `geometry` submodule that is 100% pure: no `HWND`, no Win32 call, nothing but rects and indices in, rects and booleans out. That purity is what makes it unit-testable in isolation, and the crate split is what makes the boundary enforceable rather than aspirational. One deliberate asymmetry lives there: `geometry`'s point-in-rect test is **inclusive** of the right/bottom edge, while the tray menu's and the settings window's are **exclusive**. `winspaces-win32` ships no shared `pt_in_rect` at all — not an oversight, a refusal — because unifying the two semantics would silently shift every Overview hit target (most visibly the card close button and card edges, which sit exactly on the boundary) by a pixel. Overview keeps its own, commented as deliberately different.

---

## 2. Invocation Triggers

Overview can be toggled through four distinct triggers:

| Trigger | Mechanism | Details |
| :--- | :--- | :--- |
| **Tray Icon Left-Click** | `WM_TRAYICON (WM_LBUTTONUP)` | Clicking the tray icon immediately toggles Overview (<5ms). Right-click opens the context menu (Settings via "Configure Settings..."). Double-click has no separate action — each click is an instant toggle. |
| **`Win + Tab`** | `WH_KEYBOARD_LL` | Low-level keyboard hook catches `Win+Tab` before Windows Shell/DWM and opens WinSpaces Overview. |
| **`Ctrl + Up`** | Win32 Hotkey / Keyboard Hook | macOS-native Overview hotkey. |
| **`Ctrl + Mouse Button 4/5`**| AutoHotkey / Shortcut Integration | Side mouse buttons toggle Overview. |
| **CLI / IPC Shortcut** | `winspaces.exe --overview` | Posts `WM_WINSPACES_TOGGLE_OVERVIEW` (`WM_USER + 103`) to the running daemon. Allows pinning a dedicated Overview shortcut to the Windows taskbar. |
...
1. **Never do work inside the hook.** Windows enforces a system timeout on low-level hook callbacks; exceeding it gets the hook **silently uninstalled** and `Win+Tab` interception dies until restart. The hook therefore only `PostMessageW`s `WM_WINSPACES_TOGGLE_OVERVIEW` to the daemon's message loop and returns — the overlay is built there.
...
3. Use the **WinSpaces Tray Icon** or **`Win + Tab`** as your primary Overview trigger.
4. (Optional) Create a shortcut to `winspaces.exe --overview` and pin it to your taskbar next to the Start button.
...
3. **Target Space Switching**:
   ...
   - If the window lives on an inactive space on that display, `switch_space(mon_idx, target_space, Some(target_hwnd))` transitions that monitor to the correct space, uncloaks the window, and updates the Fluent tray icon badge.
...
**The scanner hides one only when it is genuinely on screen, and the reclaim never gives visibility back.** Both halves are load-bearing, and the bug they fix is worth recording because WinSpaces was feeding it to itself.

Left alone, the system keeps every window on that list out of sight by one of two means, and the measured states say which:

| window | visible | cloaked | on screen |
|---|---|---|---|
| Task Host Window (198×56), Windows Push Notifications Platform (0×0) | no | no | no |
| Windows Input Experience (3840×2560) | **yes** | `DWM_CLOAKED_SHELL` | no |

So `WS_VISIBLE` alone does not mean "showing", and hiding a window that is already out of sight buys nothing while taking visibility from one whose owner manages it. `system_window_is_showing` (visible **and** uncloaked by anyone) is the only state worth acting on — and for a window on this list it is an anomaly, not a state to preserve.

That is why `reclaim_orphaned_windows` undoes the cloak but never calls `ShowWindow`. The `SW_SHOWNOACTIVATE` that used to run there was not restoring these windows, it was *creating* the anomaly: an unowned, non-`WS_EX_TOOLWINDOW` window handed `WS_VISIBLE` gets a taskbar button, so every exit left "Task Host Window" and "Windows Push Notifications Platform" in the taskbar — and because the corrupted state persists for the lifetime of the host process, the next scan read it as their baseline and dutifully restored it again on the next exit. Recording "was it visible when we hid it?" is precisely the wrong fix: it preserves the corruption faithfully. Not re-showing ends the loop and repairs an already-corrupted session on the next run.

### Automatic Window Scanning
On startup and whenever Overview opens, WinSpaces executes `scan_untracked_windows()` to index all running top-level application windows (Brave, VS Code, Terminal, etc.) and assign them to their respective monitor's active space.
...
3. **The card count filters at render time**, matching the overview grid's own filter exactly, so the label and the thumbnails cannot disagree regardless of the tracked list's state.
