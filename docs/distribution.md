...
`installer/winspaces.iss`, compiled by `scripts/make-installer.ps1` into `dist/WinSpaces-Setup-x64-<version>.exe`. The version is parsed from `crates/winspaces-daemon/Cargo.toml` — bump it there; nothing else encodes it.
