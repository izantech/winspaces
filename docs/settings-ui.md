...
How `winspaces.exe --settings` draws a Windows 11 Settings-style configurator with nothing but Win32, GDI, and DWM — the same technique as the tray menu ([`tray-and-menu.md`](tray-and-menu.md) §2), scaled up to a resizable top-level window. Code lives in `crates/winspaces-ui/src/settings/`, a peer of `menu` and `overview` in the same crate.
...
- **Painting** (`winspaces_win32::gdi::surface::paint_surface`): every `WM_PAINT` renders the whole client area into a premultiplied 32bpp top-down DIB — base tint at alpha 216 so Mica shows through, GDI foreground (ClearType text via `create_font`, `RoundRect` cards), then the alpha fixup promoting every GDI-touched pixel to opaque, and a single `BitBlt`. This is the *same function* the tray menu calls ([`tray-and-menu.md`](tray-and-menu.md) §2) — only the tint differs, alpha 216 here vs the menu's 232 — not a parallel copy. Full-window repaint per interaction is deliberate; Overview repaints entire screens the same way, though through the separate `double_buffer` technique (see [`overview.md`](overview.md)).
...
- **Card descriptions wrap** to at most two lines: `pages::layout` measures each description at the exact width the paint pass draws it (`measure_text_wrapped`, `DT_WORDBREAK | DT_EDITCONTROL`) and grows the card from its 68 dip minimum, re-centring the trailing control; a third line ellipsizes. Headers, nav labels and buttons stay single-line. Every string comes from the language table ([`i18n.md`](i18n.md)); the hero pill/button texts are produced once by `pages::hero_texts` for both the measure and the draw pass.
- **Combo dropdown** (the one extra HWND): a `WS_EX_NOACTIVATE | WS_EX_TOOLWINDOW | WS_EX_TOPMOST` popup reusing the window recipe (dark attr, `DWMWA_WINDOW_CORNER_PREFERENCE` round) for App Theme selection, the interface Language (row 0 follows the Windows display language, then each shipped language by its own name; committing one switches the whole window in place, retitles it and autosaves so the daemon follows), and Tiling inner/outer gap presets (`0, 4, 8, 12, 16, 24 px`). Because it never activates, the main window keeps focus and all keyboard routing (arrows move the highlight within *that* combo's item count, Enter/Space commit); dismissal needs no hooks — outside presses in the main window, `WM_NCLBUTTONDOWN`, deactivation, Esc, or scrolling all close it. Two rules keep a *click on a row* from being read as a dismiss: the popup answers `WM_MOUSEACTIVATE` with `MA_NOACTIVATE` (`DefWindowProc`'s `MA_ACTIVATE` moves activation off the settings window even though the popup cannot take it), and the main window's `WM_ACTIVATE` light-dismiss ignores a deactivation whose incoming window is the popup itself. Without either, the popup is destroyed on the button-down and the row never commits.
