...
1. **Rust Daemon (`crates/winspaces-daemon`, `winspaces.exe`)**:
   ...
   - **Mission Control (`mission_control.rs`)**: Native GPU-accelerated Exposé overlay with live DWM thumbnails (`DwmRegisterThumbnail`), native aspect-ratio preservation (`DwmQueryThumbnailSourceSize`), top Spaces bar, and drag-and-drop window relocation across spaces. Open, close, and space-switch transitions are `WM_TIMER`-driven DWM-thumbnail/GDI tweens (`docs/mission-control.md` §6) honoring both the `mission_control_animations` config toggle and the system "Show animations in Windows" setting.
